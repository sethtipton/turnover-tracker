...
-- Public maintenance capability bootstrap alignment. This mirrors
-- 20260813100000_public_maintenance_capabilities.sql for clean installs.
/* Kept as a source-note only here; the executable alignment is appended after
   the maintenance tables and RLS definitions below. */
/*
begin;
...
begin
  ...
  from public.units unit where unit.id = target_unit_id;
  if target_property_id is null or not public.can_edit_property(target_property_id, target_workspace_id) then
    raise exception 'You cannot manage this unit maintenance QR code.' using errcode = '42501';
  end if;
  loop
    attempts := attempts + 1;
    if attempts > 8 then raise exception 'A new maintenance QR code could not be generated.'; end if;
    ...
    begin
      update public.units set
        maintenance_access_token_hash = next_hash,
...
declare
  ...
  target_workspace_id uuid;
begin
  select unit.property_id, unit.workspace_id into target_property_id, target_workspace_id
  from public.units unit where unit.id = target_unit_id;
  if target_property_id is null or not public.can_edit_property(target_property_id, target_workspace_id) then
    raise exception 'You cannot manage this unit maintenance QR code.' using errcode = '42501';
  end if;
  update public.units set maintenance_access_enabled = false, updated_at = now() where id = target_unit_id;
end;
$$;

create or replace function public.resolve_public_maintenance_capability(target_token_hash text)
returns table (workspace_id uuid, property_id uuid, unit_id uuid, property_name text, unit_name text)
...
as $$
  ...
  where unit.maintenance_access_token_hash = target_token_hash and unit.maintenance_access_enabled
  limit 1;
$$;
...
as $$
  with claimed as (
    update public.units unit set maintenance_access_last_submitted_at = now()
    where unit.maintenance_access_token_hash = target_token_hash
      and unit.maintenance_access_enabled
      and (unit.maintenance_access_last_submitted_at is null or unit.maintenance_access_last_submitted_at < now() - interval '30 seconds')
  ...
  from claimed join public.units unit on unit.id = claimed.id
...
-- Executable public maintenance capability alignment (after Phase 4 tables).
begin;
drop function if exists public.resolve_maintenance_qr_token(text);
drop function if exists public.get_my_maintenance_qr_context(text);
drop function if exists public.regenerate_unit_maintenance_qr(uuid);
...
declare
  target_property_id uuid; target_workspace_id uuid; next_token text; next_hash text;
  already_issued boolean; attempts integer := 0;
begin
  select unit.property_id, unit.workspace_id, unit.maintenance_access_token_hash is not null
  into target_property_id, target_workspace_id, already_issued from public.units unit where unit.id = target_unit_id;
  if target_property_id is null or not public.can_edit_property(target_property_id, target_workspace_id) then
    raise exception 'You cannot manage this unit maintenance QR code.' using errcode = '42501';
  end if;
  loop
    ...
    next_hash := encode(extensions.digest(next_token, 'sha256'), 'hex');
    begin
      update public.units set maintenance_access_token_hash = next_hash, maintenance_access_enabled = true,
        maintenance_access_created_at = now(), maintenance_access_rotated_at = case when already_issued then now() else null end,
        maintenance_access_last_submitted_at = null, updated_at = now() where id = target_unit_id;
      return next_token;
...
declare target_property_id uuid; target_workspace_id uuid;
begin
  select unit.property_id, unit.workspace_id into target_property_id, target_workspace_id from public.units unit where unit.id = target_unit_id;
  if target_property_id is null or not public.can_edit_property(target_property_id, target_workspace_id) then
    raise exception 'You cannot manage this unit maintenance QR code.' using errcode = '42501';
  end if;
  update public.units set maintenance_access_enabled = false, updated_at = now() where id = target_unit_id;
...
language sql stable security definer set search_path = public as $$
  select unit.workspace_id, unit.property_id, unit.id, property.name, unit.name
  from public.units unit join public.properties property on property.id = unit.property_id
  where unit.maintenance_access_token_hash = target_token_hash and unit.maintenance_access_enabled limit 1;
$$;
...
language sql volatile security definer set search_path = public as $$
  ...
  select claimed.workspace_id, claimed.property_id, claimed.id, property.name, unit.name
  from claimed join public.units unit on unit.id = claimed.id join public.properties property on property.id = claimed.property_id;
$$;
...
set public = false, file_size_limit = 20971520,
    allowed_mime_types = array['image/jpeg', 'image/png', 'image/webp', 'image/heic', 'audio/webm', 'audio/mp4', 'audio/mpeg', 'audio/wav', 'audio/aac']
where id = 'maintenance-attachments';
commit;
