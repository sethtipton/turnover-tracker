...
-- maintenance_email_worker_url: full deliver-maintenance-emails function URL
...
do $$ begin
  if (select count(*) from vault.decrypted_secrets where name in
    ('maintenance_email_worker_url','maintenance_email_job_secret')) <> 2 then
    raise exception 'Configure the two maintenance email Vault secrets first';
  end if;
end $$;
select cron.schedule(
  'deliver-maintenance-emails',
  '* * * * *',
  $job$
    select net.http_post(
      url := (select decrypted_secret from vault.decrypted_secrets where name='maintenance_email_worker_url'),
      headers := jsonb_build_object('Content-Type','application/json','Authorization',
        'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name='maintenance_email_job_secret')),
      body := '{}'::jsonb,
      timeout_milliseconds := 60000
    )
    where exists(select 1 from public.maintenance_email_outbox
